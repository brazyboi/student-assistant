...
export const router = s.router(contract, {
    startStudySession: async ({ body: {topic, problem}, user}) => {
        const result = await pool.query(
            `INSERT INTO study_sessions (user_id, topic, problem)
            VALUES ($1, $2, $3)
            RETURNING *`,
            [user.id, topic, problem]
        );
        return { 
            status: 200, 
            body: result.rows[0] 
    ...
    addAttempt: async ({ body: { user_attempt }, params, user }) => {
        ...
        if (session_res.rows[0].user_id !== user.id) {
